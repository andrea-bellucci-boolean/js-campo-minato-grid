/*
Consegna

1. L'utente clicca su un bottone che genererà una griglia di gioco quadrata.
2. Ogni cella ha un numero progressivo, da 1 a 100.
Ci saranno quindi 10 caselle per ognuna delle 10 righe.
Quando l'utente clicca su ogni cella, la cella cliccata si colora di azzurro ed emetto un messaggio in console con il numero della cella cliccata.

Bonus
Aggiungere una select accanto al bottone di generazione, che fornisca una scelta tra tre diversi livelli di difficoltà:
- con difficoltà 1 => 100 caselle, con un numero compreso tra 1 e 100, divise in 10 caselle per 10 righe;
- con difficoltà 2 => 81 caselle, con un numero compreso tra 1 e 81, divise in 9 caselle per 9 righe;
- con difficoltà 3 => 49 caselle, con un numero compreso tra 1 e 49, divise in 7 caselle per 7 righe;

Consigli del giorno:  :party_wizard:
Scriviamo prima cosa vogliamo fare passo passo in italiano, dividiamo il lavoro in micro problemi.
Ad esempio:
Di cosa ho bisogno per generare i numeri?
Proviamo sempre prima con dei console.log() per capire se stiamo ricevendo i dati giusti.
Le validazioni e i controlli possiamo farli anche in un secondo momento.
*/

const playButtonEl = document.getElementById('play');
const gridContainerEl = document.getElementById('grid-container');
const levelEl = document.getElementById('level');

console.log(playButtonEl);
console.log(levelEl);

function createSquare(squareNumber, numeroColonne) {
  const squareEl = document.createElement('div');
  squareEl.style.width = `calc(100% / ${numeroColonne})`;
  squareEl.style.height = `calc(100% / ${numeroColonne})`;
  squareEl.classList.add('square');
  squareEl.textContent = squareNumber;

  return squareEl;
}

function generateGrid(gridEl, numeroQuadrati, numeroColonne) {
  for (let i = 1; i <= numeroQuadrati; i++) {
    const squareEl = createSquare(i, numeroColonne);
    // Quando l'utente clicca su ogni cella, la cella cliccata si colora di azzurro ed emetto un messaggio in console con il numero della cella cliccata.
    squareEl.addEventListener('click', function () {
      squareEl.classList.toggle('selected');
      console.log(i);
    });

    gridEl.append(squareEl);
  }
}

function getSquareNumber(level) {
  if (level == 1) {
    // Easy
    return 100;
  } else if (level == 2) {
    // Hard
    return 81;
  } else if (level == 3) {
    // Crazy
    return 49;
  }
}

playButtonEl.addEventListener('click', function () {
  // Inizio gioco.
  console.log('Siamo dentro la function!');
  gridContainerEl.innerHTML = '';

  const gridEl = document.createElement('div');
  gridEl.classList.add('grid');

  const level = levelEl.value;
  console.log(level);

  const squareNumber = getSquareNumber(level);
  const columnNumber = Math.sqrt(squareNumber);

  generateGrid(gridEl, squareNumber, columnNumber);

  gridContainerEl.append(gridEl);
});
